...
export async function generateMetadata(): Promise<Metadata> {
  return buildMetadata({ title: 'Datenschutz', path: '/datenschutz' })
}

export default async function PrivacyPage() {
  const chrome = await renderChrome()
  const html = sanitizeRichText(chrome.settings['legal.privacy'])

  return (
    <>
      <main id="main" className="container-content py-16 sm:py-24">
        <div className="mx-auto max-w-prose">
          <h1 className="font-display text-[30px] font-semibold tracking-tight sm:text-[38px]">Datenschutz</h1>
          {html ? (
            <div className="rich-text mt-8" dangerouslySetInnerHTML={{ __html: html }} />
          ) : (
            <p className="mt-8 text-[15px] leading-relaxed text-fg-muted">
              Die Datenschutzerklärung wird derzeit ergänzt. Bitte wenden Sie sich in der Zwischenzeit direkt an{' '}
